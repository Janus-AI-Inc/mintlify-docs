...
async def create_webhook_target_agent(
    ...
    workflow_id: str, 
    payload: dict
) -> Callable[[int], Awaitable[MultimodalOutput]]
```
...
async def main():
    ...
    # Run evaluations with webhook trigger
...
The webhook target agent now returns `MultimodalOutput` with structured data from your workflows:
...
async def main():
    ...
    # Run evaluations - agent returns MultimodalOutput
    results = await run_simulations(
        num_simulations=5,
        max_turns=1,
        target_agent=target_agent,
...
- Returns the final result as a `MultimodalOutput` with structured data, files, and metadata

### Related Functions

<Card title="run_simulations" icon="play" href="/api-reference/run-simulations">
  Run evaluations with webhook triggers
</Card>
