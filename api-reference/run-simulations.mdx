...
description: "Run automated evaluations on your AI agent"
---

## run_simulations

Run automated evaluations on your AI agent's performance across various tasks.
...
- Increase test count gradually as you optimize your agent
- Monitor API rate limits for large-scale testing
</Tip>

<Warning>
Running many tests can be resource-intensive. Monitor your system resources and API usage.
</Warning>
...
<Info>
Webhook testing allows you to evaluate entire workflows end-to-end by triggering them via HTTP webhooks, then monitoring their execution and results.
</Info>

### Multimodal Output Support

The SDK now supports multimodal outputs, allowing agents to return structured data, files, and metadata:
...
class MyMultimodalAgent:
    async def chat(self, prompt: str) -> MultimodalOutput:
        return MultimodalOutput(
            ...
            json_data={"case_id": "CASE-123", "status": "completed"},
            files=[FileAttachment(name="report.pdf", content=pdf_bytes, mime_type="application/pdf", size=len(pdf_bytes))],
...
<Card title="multimodal-output" icon="layers" href="/api-reference/multimodal-output">
  Handle structured data, files, and metadata in agent responses
</Card>
</CardGroup> 